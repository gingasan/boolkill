...
class ReclorProcessor:
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(self._read(os.path.join(data_dir, "train.json")), "train")

    def get_dev_examples(self, data_dir):
        return self._create_examples(self._read(os.path.join(data_dir, "val.json")), "dev")

    def get_test_examples(self, data_dir):
        return self._create_examples(self._read(os.path.join(data_dir, "test.json")), "test")
    ...
    def _create_examples(lines, set_type):
        examples = [
            InputExample(
                ...
                contexts=[line["context"], line["context"], line["context"], line["context"]],
                endings=line["answers"],
...
class LogiqaProcessor(object):
    """Processor for the LogiQA data set."""

    def get_train_examples(self, data_dir):
        return self._create_examples(self._read(os.path.join(data_dir, "train.txt")), "train")

    def get_dev_examples(self, data_dir):
        return self._create_examples(self._read(os.path.join(data_dir, "dev.txt")), "dev")

    def get_test_examples(self, data_dir):
        return self._create_examples(self._read(os.path.join(data_dir, "test.txt")), "test")
    ...
    def _create_examples(lines, set_type):
        examples = [
            InputExample(
                guid="%s-%s" % (set_type, i),
                question=line["question"],
                contexts=[line["text"], line["text"], line["text"], line["text"]],
                endings=line["options"],
                label=str(line["answer"]),
            )
            for i, line in enumerate(lines)
        ]
...
def main():
    ...
    parser.add_argument("--task_name", type=str, default="ReClor",
